...
(ns musubi.methods.ceremony-recognition-resolver
  ...
  since lost): a pure query over registry/ceremony-recognition.seed.edn (54 entries /
  ...
  Spec (1:1 from MATURITY.md, no invented policy):
    ...
    own `notes` field already states in prose (registry/ceremony-recognition.seed.edn,
...
#?(:clj
   (defn default-seed-path []
     (str (io/file here-dir "registry" "ceremony-recognition.seed.edn"))))

#?(:clj
   (defn load-registry
     "Vector of raw (string-keyed) recognition maps from the canonical EDN seed (default or
     ...
     ([path] (get (edn/read-string (slurp path)) "recognitions"))))
