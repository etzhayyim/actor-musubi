(ns musubi.methods.social
  "social.cljc — 結 — Covenant Ceremony DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from musubi's own
...
(def DISCLAIMER
  (str "【観測ミラー / accountability map — NOT a verdict, NOT advice, 非断定】 "
       "結 — Covenant Ceremony が既知の観測から編んだ事実の要約です。"))
...
(defn draft-observation-post
  "A dry-run observation post over musubi's own domain (Non-profit religious-corp covenant ceremony performance substrate. TIGHT PAIR of chigiri (chigiri attests on-chain; musubi performs…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "musubi R0: live social posting is Council Lv6+ + operator + "
